...
function runMigrations(PDO $pdo): array {
    ...
    ");
    $steps[] = "sale_items table ready";

    $seedStaffStmt = $pdo->prepare("
        INSERT INTO staff (name, icon)
        SELECT * FROM (
            SELECT 'Staff 1', '🧔' UNION ALL
            SELECT 'Staff 2', '👨' UNION ALL
            SELECT 'Staff 3', '👩'
        ) AS tmp
        WHERE NOT EXISTS (SELECT 1 FROM staff LIMIT 1)
    ");
    $seedStaffStmt->execute();
    $steps[] = "default staff seeded if empty";

    $seedServiceStmt = $pdo->prepare("
        INSERT INTO services (name, price, icon, sort_order)
        SELECT * FROM (
            SELECT 'Hair Cut', 150.00, '✂️', 1 UNION ALL
            SELECT 'Shave', 80.00, '🪒', 2 UNION ALL
            SELECT 'Hair Wash', 100.00, '🚿', 3 UNION ALL
            SELECT 'Facial', 300.00, '🧴', 4
        ) AS tmp
        WHERE NOT EXISTS (SELECT 1 FROM services LIMIT 1)
    ");
    $seedServiceStmt->execute();
    $steps[] = "default services seeded if empty";

    return $steps;
}

function collectDbStats(PDO $pdo, array $tables): array {
    $stats = [];
    $existing = 0;
    ...
    foreach ($tables as $table) {
        ...
        $stats[$table] = ['exists' => $exists, 'rows' => $rows];
    }
